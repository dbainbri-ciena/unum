...
func (c *SwarmClient) GetInfo(labels map[string]string) (*ClusterInfo, error) {
	info := ClusterInfo{}
	...
	for _, service := range services {
		...
		if labelMatch(service.Spec.Labels, labels) {
			...
			for _, task := range tasks {
				...
				log.Debugf("Found matching task '%s' on node '%s'", task.ID, task.NodeID)
				 ...
				 */
				found := false
				count := len(task.NetworksAttachments)
				if count == 1 {
				...
				for _, network := range task.NetworksAttachments {
					if labelMatch(network.Network.Spec.Labels, labels) ||
						(count == 2 && network.Network.Spec.Name != "ingress") {
	...
	return nil, fmt.Errorf("Service matching specified labels, '%s', not found",
		labels)
}
