...
type configSpec struct {
	...
	Labels       map[string]string `envconfig:"LABELS" desc:"labesl to match service" default:"org.onosproject.onos.cluster:true"`
...
func main() {
	...
	info, err := orch.GetInfo(config.Labels)
	if err != nil {
	...
	for {
		...
		if err != nil {
			...
			if info.Expected == uint64(len(info.Nodes)) && info.Expected >= 3 {
				GenerateConfig(info, listener.Update)
			}
		}
